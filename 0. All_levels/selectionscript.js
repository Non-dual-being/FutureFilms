...
let hoverTimeout;
    ...
    const downloadButtonContainer = downloadButton.parentElement; // Neem aan dat de knop een container heeft
    const startKnopContent = document.querySelector('.start-knop-content');

    if (downloadButton && downloadButtonContainer) {
        // Voeg een eventlistener toe om te controleren of er overlap is bij hover op de container
        downloadButtonContainer.addEventListener('mouseenter', function () {
            if (checkOverlap(downloadButtonContainer, startKnopContent)) {
                downloadButton.style.zIndex = '1000'; // Zet de downloadknop boven de startknop
            } else {
                downloadButton.style.zIndex = ''; // Reset z-index als er geen overlap is
            }
        });

        downloadButtonContainer.addEventListener('mouseleave', function () {
            downloadButton.style.zIndex = ''; // Reset z-index bij mouseleave
        });
    } else {
        console.log('You wil never see this message if think your better then AI(#king koen)');
    }

    // Functie om te controleren of er overlap is tussen twee elementen
    function checkOverlap(element1, element2) {
        ...
        // Controleer of er een overlap is tussen de rechthoeken van de twee elementen
